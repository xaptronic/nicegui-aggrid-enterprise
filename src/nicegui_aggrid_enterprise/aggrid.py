...
class AgGridEnterprise(
    Element,
    component="aggrid_enterprise.js",
    esm={"nicegui-aggrid-enterprise": "dist"},
    default_classes="nicegui-aggrid",
):
    """AG Grid Enterprise with community charts support.

    Use this class for AG Grid Enterprise features with AG Charts Community.
    For AG Charts Enterprise features, use AgGridEnterpriseCharts instead.
    """
    ...
    def __init__(
        ...
        *,
        ...
        An element to create a grid using `AG Grid Enterprise <https://www.ag-grid.com/>`_.
        Updates can be pushed to the grid by updating the ``options`` property.

        The methods ``run_grid_method`` and ``run_row_method`` can be used to interact
        with the AG Grid instance on the client.
        ...
        self._props["license-key"] = self.__class__.license_key
    ...
    def from_pandas(
        ...
        *,
        ...
        Note:
        If the DataFrame contains non-serializable columns of type ``datetime64[ns]``,
        ``timedelta64[ns]``, ``complex128`` or ``period[M]``, they will be converted
        to strings. To use a different conversion, convert the DataFrame manually
        before passing it to this method.
        See `issue 1698 <https://github.com/zauberzeug/nicegui/issues/1698>`_ for more
        information.

        :param df: Pandas DataFrame
        :param html_columns: list of columns that should be rendered as HTML
        :param theme: AG Grid theme (default: ``options['theme']`` or "quartz")
        :param auto_size_columns: whether to automatically resize columns
    ...
    def from_polars(
        ...
        *,
        ...
        To use a different conversion, convert the DataFrame manually before passing it
        to this method.
    ...
    @theme.setter
    def theme(self, value: ThemeType) -> None:
    ...
    def auto_size_columns(self) -> bool:
        ...
        return (
            self._props["options"].get("autoSizeStrategy", {}).get("type")
            == "fitGridWidth"
        )
    ...
    def run_grid_method(
        ...
        See `AG Grid API <https://www.ag-grid.com/javascript-data-grid/grid-api/>`_
    ...
    def run_row_method(
        ...
        See `AG Grid Row Reference <https://www.ag-grid.com/javascript-data-grid/row-object/>`_
        for a list of methods.
    ...
    async def get_selected_rows(self) -> list[dict]:
        ...
        This method is especially useful when the grid is configured with
        ``rowSelection: 'multiple'``.

        See `AG Grid API
        <https://www.ag-grid.com/javascript-data-grid/row-selection/>`_
    ...
    async def get_client_data(
        ...
        *,
        ...
        See `AG Grid API <https://www.ag-grid.com/javascript-data-grid/accessing-data/>`_
        for more information.

        Note that when editing a cell, the row data is not updated until the cell exits
        the edit mode. This does not happen when the cell loses focus, unless
        ``stopEditingWhenCellsLoseFocus: True`` is set.

        :param timeout: timeout in seconds (default: 1 second)
        :param method: method to access the data (default: "all_unsorted")
...
class AgGridEnterpriseCharts(
    Element,
    component="aggrid_enterprise_charts.js",
    esm={"nicegui-aggrid-enterprise-charts": "dist-enterprise-charts"},
    default_classes="nicegui-aggrid",
):
    """AG Grid Enterprise with enterprise charts support.

    Use this class for AG Grid Enterprise with AG Charts Enterprise.
    For AG Charts Community (smaller bundle), use AgGridEnterprise instead.
    """
    ...
    def __init__(
        ...
        *,
        ...
        """AG Grid Enterprise with Charts

        An element to create a grid using `AG Grid Enterprise <https://www.ag-grid.com/>`_
        with AG Charts Enterprise support.
        Updates can be pushed to the grid by updating the ``options`` property.
